...
// -------- Geocoding (for Telegram WebView) --------
// Telegram WebView can block direct calls to Nominatim from the browser.
// So we proxy geocoding through our backend (same origin) with required User-Agent.
async function geocode(q) {
  ...
  const r = await fetch(url, {
    headers: {
      "User-Agent": "PayTaksiTelegram/1.0 (telegram mini app)",
