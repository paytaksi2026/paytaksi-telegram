...
await db._query(`CREATE TABLE IF NOT EXISTS driver_locations (
  ...
  ensureColumn('users', 'car_make', 'car_make TEXT');
  ensureColumn('users', 'car_model', 'car_model TEXT');
...
app.post('/api/driver/rate', async (req,res)=>{
  try{
    ...
    if(r<1 || r>5){
      return res.json({success:false,error:"BAD_RATING"});
    }
    ...
    await pool.query(
      "INSERT INTO driver_ratings(driver_phone,rating,created_at) VALUES($1,$2,$3)",
      [driver,r,Date.now()]
    );
...
app.get('/api/driver/rating', async (req,res)=>{
  try{
    ...
    const avg = Number(q.rows[0].avg || 0);
